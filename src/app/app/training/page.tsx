...
export default function TrainingPage() {
	...
	const [step, setStep] = useState<"original" | "correction" | "audio">(
		"original",
	);
	...
	const loadSubmissions = useCallback(
		async (userIdToLoad: string, force = false) => {
			...
			try {
				...
				const formattedSubmissions: StoredSubmission[] = data.submissions.map(
			...
			try {
				...
				if ("completed" in data && data.completed) {
					toast({
						...
						description:
							data.message || "You have completed all available stories!",
					});
				...
				if (
					!options?.force &&
					lastStoryId.current &&
					data.storyId === lastStoryId.current
				) {
					setIsStreaming(false);
					return;
				}
		...
		return (
			<div className="min-h-screen bg-background p-4 md:p-6 flex items-center justify-center">
				<Loader2
					...
					aria-label="Loading training experience"
				/>
			</div>
		);
	}
					...
					<DialogHeader>
						...
						<DialogDescription className="text-base space-y-3">
							...
							<p className="font-semibold text-foreground">
								...
								<span className="text-destructive text-lg">{countdown}</span>{" "}
				...
				<div className="max-w-7xl mx-auto mb-10">
					<div className="flex flex-col gap-4 rounded-[2rem] border border-slate-200 bg-white px-4 py-4 shadow-xl shadow-slate-200/70 backdrop-blur-sm md:flex-row md:items-center md:justify-between md:px-4">
						...
						<div className="inline-flex items-center gap-2 rounded-xl bg-slate-100 px-4 py-2">
							...
							<span className="text-sm font-medium text-zinc-400/80">
								{totalEarnings * 100} Points
							</span>
						</div>
				...
				<div className="max-w-3xl mx-auto">
					<div className="rounded-[2.5rem] border border-slate-200 bg-white px-6 py-8 shadow-2xl shadow-slate-200/60 backdrop-blur-sm md:px-10 md:py-12">
						...
						>
							<StepIndicator
								currentStep={step}
								isAudioComplete={Boolean(audioBlob)}
							/>
							...
							<div className="relative">
								<div
									className={`space-y-4 transition-all duration-200 ease-in-out ${
										step === "audio"
											? "absolute inset-x-0 -translate-y-12 opacity-0"
											...
											<>
												<div className="space-y-1 px-1">
													...
													<p className="text-xs text-slate-500 md:text-sm">
														Correct any errors and adjust the text to match your
														dialect
													</p>
												</div>
												<AutoResizeTextarea
									...
									<div className="mt-8 space-y-6 rounded-3xl border border-blue-100 bg-gradient-to-b from-blue-50 via-white to-blue-50 p-6 shadow-2xl shadow-blue-100 md:p-8">
										...
										<div
											className={`transition-all duration-200 ease-in-out ${
												step === "audio"
													? "translate-y-0 opacity-100"
													: "h-0 translate-y-12 overflow-hidden opacity-0"
											}`}
										>
											<AudioRecorder
												key={originalText}
