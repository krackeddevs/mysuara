	...
	return (
		<main className="min-h-[calc(100vh-80px)] w-full bg-[var(--bg)] text-[var(--ink)]">
			<div className="relative isolate mx-auto flex w-full max-w-7xl flex-col gap-20 px-4 pb-28 pt-24 sm:px-6 lg:px-10">
					...
					<div className="relative flex flex-col gap-12 lg:flex-row lg:items-center lg:justify-between">
						<div className="max-w-xl space-y-6">
							...
							<p className="text-lg leading-relaxed text-[var(--muted)]">
								Every milestone is co-designed with councils, community
								partners, and enterprises so Suara delivers inclusive voice
								automation that Malaysia can trust.
							</p>
							<ul className="grid gap-2 text-sm text-[#1d3350]">
								{heroHighlights.map((highlight) => (
									...
									>
										<span
											className="mt-1 inline-flex h-2.5 w-2.5 flex-none rounded-full bg-[#2563eb]"
											aria-hidden
										/>
										<span>{highlight}</span>
									</li>
				...
				>
					<header className="mx-auto max-w-3xl space-y-3 text-center">
						...
						<p className="text-base text-[#1d3350b3]">
							Each quarter advances dialect coverage, operational excellence,
							ecosystem reach, and measurable impact.
						</p>
					</header>
					<div className="grid gap-6 sm:grid-cols-2">
						{roadmapPhases.map((quarter, index) => (
							...
							>
								<div
									className="absolute inset-0 bg-gradient-to-br from-[#2563eb14] via-transparent to-transparent opacity-0 transition group-hover:opacity-100"
									aria-hidden
								/>
								<div className="relative flex items-center justify-between">
									...
									<span className="text-xs font-semibold uppercase tracking-[0.28em] text-[#1d3350b3]">
										Phase {index + 1}
									</span>
								</div>
				...
				>
					<div
						className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(37,99,235,0.12),_transparent_52%)] opacity-70"
						aria-hidden
					/>
					<div className="relative grid gap-10 lg:grid-cols-[1fr_1fr] lg:items-center">
						<div className="space-y-6">
							...
							<h2 className="text-[clamp(30px,3vw,44px)] font-semibold leading-[1.12] text-[var(--ink)]">
								A phased plan that scales from dialect coverage to measurable
								outcomes.
							</h2>
							<p className="text-base leading-relaxed text-[#1d3350b3]">
								Each phase spans multiple releases, pilot cohorts, and audit
								cycles so partners can adopt confidently at national scale.
							</p>
						</div>
						<div className="grid gap-6">
							{milestoneTimeline.map((phase) => (
								<article
									key={phase.title}
									className="group relative flex gap-5 rounded-[28px] border border-[#2563eb1a] bg-white/90 p-6 shadow-[0_22px_54px_rgba(4,9,20,0.08)] backdrop-blur"
								>
									<div
										className={`relative grid h-20 w-20 place-items-center overflow-hidden rounded-2xl bg-gradient-to-br ${phase.accent} border border-[#2563eb1a]`}
									>
										<Image
											src={phase.icon}
											alt={phase.title}
									...
									<div className="flex flex-col gap-2">
										<span className="text-xs font-semibold uppercase tracking-[0.3em] text-[#2563eb80]">
											{phase.label}
										</span>
				...
				>
					<div className="space-y-5">
						...
						<p className="text-base leading-relaxed text-[#1d3350b3]">
							Even when timelines flex, these principles stay constant. They
							help every partner trust the plan and the pace.
						</p>
					</div>
					<div className="grid gap-6">
						{guidingPrinciples.map((theme) => (
							<article
								key={theme.title}
								className="rounded-[26px] border border-[#2563eb1f] bg-[#f8fbff] p-7 shadow-[0_22px_50px_rgba(4,9,20,0.08)]"
							>
								<h3 className="text-lg font-semibold text-[var(--ink)]">
									{theme.title}
								</h3>
				...
				>
					<header className="mx-auto max-w-3xl space-y-3 text-center">
						...
						<p className="text-base text-[#1d3350b3]">
							Structured programmes keep your teams looped into design sprints,
							governance reviews, and launch playbooks.
						</p>
					</header>
					<div className="grid gap-6 sm:grid-cols-3">
						{partnerProgrammes.map((programme) => (
							...
							>
								<div
									className="pointer-events-none absolute inset-0 bg-gradient-to-br from-[#2563eb14] via-transparent to-transparent opacity-0 transition group-hover:opacity-100"
									aria-hidden
								/>
								<h3 className="relative text-lg font-semibold text-[var(--ink)]">
									{programme.title}
								</h3>
				...
				>
					<header className="mx-auto max-w-3xl space-y-3 text-center">
						...
						<p className="text-base text-[#1d3350b3]">
							We align priorities, co-design solutions, measure outcomes, and
							scale with governance baked in.
						</p>
					</header>
					...
					<div className="relative flex flex-col items-start gap-8 text-left sm:flex-row sm:items-center sm:justify-between">
						<div className="max-w-xl space-y-4">
							...
							<p className="text-sm leading-relaxed text-white/80">
								Invite us into your next service initiative, policy programme,
								or community rollout. Together we’ll design, deploy, and scale
								sovereign voice solutions that deliver measurable change.
							</p>
						</div>
						<div className="flex flex-wrap gap-3">
							<Link
