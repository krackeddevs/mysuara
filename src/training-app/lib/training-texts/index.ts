...
const batchEntries = rawBatchEntries.map(
	([batchNumber, stories]) =>
		[batchNumber, stories.filter(isValidStory)] as const,
);

export const allStories: TrainingStory[] = batchEntries.flatMap(
	([, stories]) => stories,
);

const batchMap: Record<number, TrainingStory[]> = Object.fromEntries(
	batchEntries,
) as Record<number, TrainingStory[]>;
